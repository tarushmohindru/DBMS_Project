...
export async function getAllLogs(filters?: { verification_status?: string; company_id?: number; plant_id?: number }) {
  let query = `
...
export async function getLogById(log_id: number) {
...
export async function createLog(company_id: number, input: CreateProductionLogInput) {
  // Verify the plant exists and is Approved before accepting a log
  const plantCheck = await pool.query(
    `SELECT status, company_id FROM EnergyPlant WHERE plant_id = $1`,
    [input.plant_id]
  );
  ...
  const duplicateCheck = await pool.query(
    `SELECT log_id FROM ProductionLog
     WHERE plant_id = $1 AND log_date = $2`,
    [input.plant_id, input.log_date]
  );
  ...
  const result = await pool.query(
    `INSERT INTO ProductionLog (plant_id, log_date, energy_kwh)
     VALUES ($1, $2, $3)
     RETURNING *`,
    [input.plant_id, input.log_date, input.energy_kwh]
  );
...
export async function verifyLog(
  log_id:       number,
  authority_id: number,
  input:        VerifyProductionLogInput
) {
  // PROCEDURE manages its own transaction internally
  await pool.query(
    'CALL verify_production_log($1, $2, $3, $4)',
    [log_id, authority_id, input.decision, input.remarks ?? null]
  );
