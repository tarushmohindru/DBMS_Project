...
export async function getWallet(company_id: number) {
  const result = await pool.query(
    `SELECT cw.*, c.name AS company_name
...
export async function getWalletBalance(company_id: number): Promise<number> {
  const result = await pool.query(
    'SELECT get_company_wallet_balance($1) AS balance',
    [company_id]
  );
  return parseFloat(result.rows[0].balance);
}

export async function getWalletHistory(company_id: number) {
  const credits = await pool.query(
    `SELECT 'CREDIT_ISSUED' AS type, cc.issued_date AS date,
  ...
  const bought = await pool.query(
    `SELECT 'PURCHASE' AS type, t.txn_date AS date,
  ...
  const sold = await pool.query(
    `SELECT 'SALE' AS type, t.txn_date AS date,
            -t.quantity AS amount, t.quantity,
            c.name AS counterparty,
            t.total_amount AS monetary_amount
     FROM   Transaction t
