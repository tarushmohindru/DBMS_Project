...
exports.adminCancelListing = adminCancelListing;
exports.getActiveListingsCursor = getActiveListingsCursor;
const db_1 = require("../config/db");
async function getActiveListings() {
    const result = await db_1.pool.query(`SELECT listing_id, seller_id, seller_name, seller_reg_no, credit_id,
...
async function getListingById(listing_id) {
    const result = await db_1.pool.query(`SELECT ml.*, c.name AS seller_name, cc.credits_issued AS original_credits
     FROM   MarketListing ml
     JOIN   Company c       ON ml.seller_id  = c.company_id
     JOIN   CarbonCredit cc ON ml.credit_id  = cc.credit_id
     WHERE  ml.listing_id = $1`, [listing_id]);
    return result.rows[0] ?? null;
}
async function createListing(seller_id, input) {
    // Verify the credit belongs to the seller and has enough unlisted quantity
    const creditCheck = await db_1.pool.query(`SELECT cc.credits_issued, cc.company_id,
            COALESCE(
     ...
     WHERE  cc.credit_id = $1`, [input.credit_id]);
    ...
    const result = await db_1.pool.query(`INSERT INTO MarketListing (seller_id, credit_id, quantity, price_per_credit)
     VALUES ($1, $2, $3, $4)
     RETURNING *`, [seller_id, input.credit_id, input.quantity, input.price_per_credit]);
    return result.rows[0];
}
async function cancelListing(listing_id, seller_id) {
    const listing = await getListingById(listing_id);
    if (!listing)
        throw { status: 404, message: 'Listing not found', code: 'NOT_FOUND' };
    ...
    const result = await db_1.pool.query(`UPDATE MarketListing SET status = 'Cancelled' WHERE listing_id = $1 RETURNING *`, [listing_id]);
    return result.rows[0];
}
async function adminCancelListing(listing_id) {
    const result = await db_1.pool.query(`UPDATE MarketListing SET status = 'Cancelled'
...
async function getActiveListingsCursor() {
    const result = await db_1.pool.query('SELECT * FROM get_active_listings_cursor()');
    return result.rows;
}
